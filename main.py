...
if __name__=="__main__":
    ...
    #Step4. record imformation of select area
    floor_object = []

    

    for LWPOLYLINE in msp.query("LWPOLYLINE[layer=='{}']".format("SelectArea")):
        ...
        floor_object.append(new_floor)

    for TEXT in msp.query("TEXT[layer=='{}']".format("floor_name")):
        get_placement = TEXT.dxf.insert
        point = CPoint(get_placement[0],get_placement[1])
        text_py = TEXT.dxf.text
        for idx, f in enumerate(floor_object):
            if f.x_min < point.x and point.x < f.x_max and f.y_min < point.y and point.y < f.y_max:
                floor_object[idx].name = text_py
                break

    for MTEXT in msp.query("MTEXT[layer=='{}']".format("floor_name")):
        get_placement = MTEXT.dxf.insert
        point = CPoint(get_placement[0],get_placement[1])
    ...
    for LWPOLYLINE in msp.query("LWPOLYLINE[layer=='{}']".format("41-樓地板面積")):
        ...
        for idx, f in enumerate(floor_object):
            if f.x_min < points[0].x and points[0].x < f.x_max and f.y_min < points[0].y and points[0].y < f.y_max:
                ...
                #****************Be careful unit conversion**********
                floor_object[idx].area_floor += sum_detA
                break
    ...
    for LWPOLYLINE in msp.query("LWPOLYLINE[layer=='{}']".format("42-梯廳面積")):
        ...
        for idx, f in enumerate(floor_object):
            if f.x_min < points[0].x and points[0].x < f.x_max and f.y_min < points[0].y and points[0].y < f.y_max:
                ...
                sum_detA = abs(detA_sum(IntPointNum,points).result())*0.0001
                #****************Be careful unit conversion**********
                floor_object[idx].area_hall += sum_detA
                break
    ...
    for LWPOLYLINE in msp.query("LWPOLYLINE[layer=='{}']".format("43-管委會面積")):
        ...
        for idx, f in enumerate(floor_object):
            if f.x_min < points[0].x and points[0].x < f.x_max and f.y_min < points[0].y and points[0].y < f.y_max:
                ...
                sum_detA = abs(detA_sum(IntPointNum,points).result())*0.0001
                #****************Be careful unit conversion**********
                floor_object[idx].area_committee += sum_detA
                break
    ...
    for LWPOLYLINE in msp.query("LWPOLYLINE[layer=='{}']".format("44-機電面積")):
        ...
        for idx, f in enumerate(floor_object):
            if f.x_min < points[0].x and points[0].x < f.x_max and f.y_min < points[0].y and points[0].y < f.y_max:
                ...
                sum_detA = abs(detA_sum(IntPointNum,points).result())*0.0001
                #****************Be careful unit conversion**********
                floor_object[idx].area_electromechanical += sum_detA
                break
    ...
    for LWPOLYLINE in msp.query("LWPOLYLINE[layer=='{}']".format("45-車道面積")):
        ...
        for idx, f in enumerate(floor_object):
            if f.x_min < points[0].x and points[0].x < f.x_max and f.y_min < points[0].y and points[0].y < f.y_max:
                ...
                sum_detA = abs(detA_sum(IntPointNum,points).result())*0.0001
                #****************Be careful unit conversion**********
                floor_object[idx].area_carramp += sum_detA
                break
    ...
    for LWPOLYLINE in msp.query("LWPOLYLINE[layer=='{}']".format("46-陽台面積")):
        ...
        for idx, f in enumerate(floor_object):
            if f.x_min < points[0].x and points[0].x < f.x_max and f.y_min < points[0].y and points[0].y < f.y_max:
                ...
                sum_detA = abs(detA_sum(IntPointNum,points).result())*0.0001
                #****************Be careful unit conversion**********
                floor_object[idx].area_balcony += sum_detA
                break
    ...
    for idx, f in enumerate(floor_object):
        cell = str(f"A{idx+2}")
        s1[cell].value = floor_object[idx].name

    s1['B1'].value = '樓地版面積'
    s1['B1'].fill = PatternFill(fill_type="solid", fgColor="FDE9D9")
    for idx, f in enumerate(floor_object):
        cell = str(f"B{idx+2}")
        s1[cell].value = floor_object[idx].area_floor
    ...
    for idx, f in enumerate(floor_object):
        cell = str(f"C{idx+2}")
        s1[cell].value = floor_object[idx].area_hall
    ...
    for idx, f in enumerate(floor_object):
        cell = str(f"D{idx+2}")
        s1[cell].value = floor_object[idx].area_electromechanical
    ...
    for idx, f in enumerate(floor_object):
        cell = str(f"E{idx+2}")
        s1[cell].value = floor_object[idx].area_committee
    ...
    for idx, f in enumerate(floor_object):
        cell = str(f"F{idx+2}")
        s1[cell].value = floor_object[idx].area_balcony
    ...
    for idx, f in enumerate(floor_object):
        cell = str(f"G{idx+2}")
        s1[cell].value = floor_object[idx].area_carramp
    ...
    for idx, f in enumerate(floor_object):
        cell = str(f"H{idx+2}")
        s1[cell].value = floor_object[idx].over_hall
    ...
    for idx, f in enumerate(floor_object):
        cell = str(f"I{idx+2}")
        s1[cell].value = floor_object[idx].over_balcony
    ...
    for idx, f in enumerate(floor_object):
        cell = str(f"J{idx+2}")
        s1[cell].value = floor_object[idx].over_hallBalcony
        s1[cell].fill = PatternFill(fill_type="solid", fgColor="D8E4BC")

    s1['K1'].value = '容積樓地板面積'
    for idx, f in enumerate(floor_object):
        cell = str(f"K{idx+2}")
        s1[cell].value = floor_object[idx].area_volumn
